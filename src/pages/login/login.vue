	...
	<view class="content">
		<view class="input-group">
			...
			<view class="input-row">
				<image @click="checkCodePaint()" v-bind:src="checkCodeSource" class="title"></image>
...
export default {
	...
	methods: {
		...
		bindLogin() {
			...
			const data = {
				userId: that.account,
				passWord: cryptoJs.MD5(that.password).toString(),
			...
			uni.$bkHttpHandler({
				...
				success: function(res) {
					...
					if (res.data.code == '100') {
						...
						uni.showToast({
							icon: 'none',
							title: res.data.msg||res.res.data.msg
						});
					}
				},
				fail: function(err) {
		...
		checkCodePaint: function() { 
			...
			uni.$bkHttpHandler({
				...
				success: function(data) {
					that.checkCodeSource = data.data;
				},
				fail: function(err) {
					debugger;
				}
